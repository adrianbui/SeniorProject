...
//     }
     let diagnostics: doc.Diagnostic[] = [];
...
//                     diagnostics.push(creatDiaContainsInMiddle(doc, lineOfText, line))
//                 }
//             }
//             if (lineOfText.text.includes("|all:")) {
//                 if (!lineOfText.text.match(/\|all:\s*$/)) {
//                     diagnostics.push(creatDiaSingleAll(doc, lineOfText, line))
//                 }
//             }
//             if (lineOfText.text.match(/^title:.{71,}/)) {
//                 diagnostics.push(creatDiaTitleTooLong(doc, lineOfText, line))
//             }
//             if (lineOfText.text.match(/^description:.{0,32}$/)) {
//                 if (!lineOfText.text.match(/^description:\s+\|\s*$/)) {
//                     diagnostics.push(creatDiaDescTooShort(doc, lineOfText, line))
//                 }
//             }
//             if (lineOfText.text.match(/[\s]+$/)) {
//                 diagnostics.push(creatDiaTrailingWhitespace(doc, lineOfText, line))
//             }
//         }
//     }
...
//     const range = new vscode.Range(line, index, line, index + indexLength)
...
//         range = new vscode.Range(line, lineOfText.text.length - l, line, lineOfText.text.length)
//     }
...
function handleDiagnostics(doc:TextDocument) {
    const lines = doc.getText().split('\n');
    for (let i = 0; i < doc.lineCount; i++) {
        const line = lines[i];
        consoledoc.getText(Range.create(i,0,i,line.length))
    // Check for Errors Here
    // Check modifiers
    // if (line.includes("contains|")) {
    //     if (!line.includes("contains|all:")) {
    //         diagnostics.push(creatDiaContainsInMiddle(doc, lines, line))
    //     }
    // }
    // if (line.includes("|all:")) {
    //     if (!line.match(/\|all:\s*$/)) {
    //         diagnostics.push(creatDiaSingleAll(doc, lines, line))
    //     }
    // }
    if (line.match(/^title:.{71,}/)) {
        diagnostics.push(creatDiaTitleTooLong(doc, lines, i));
    }
    // if (line.match(/^description:.{0,32}$/)) {
    //     if (!line.match(/^description:\s+\|\s*$/)) {
    //         diagnostics.push(creatDiaDescTooShort(doc, lines, line))
    //     }
    // }
    // if (line.match(/[\s]+$/)) {
    //     diagnostics.push(creatDiaTrailingWhitespace(doc, lines, line))
    // }
}
}
